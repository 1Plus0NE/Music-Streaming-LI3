...
void query6(char* idUser, char* year, char* n, GestorHistory* gestorHistory, GestorMusic* gestorMusic, char delimiter, FILE* output){
    ...
    char* totalListTime = wrapTotalListTime(query6Data->wrap, &totalGenre); // Altera o valor do genero para o + ouvido
    ...
    char* nMusics = wrapTotalMusics(query6Data->wrap);
    //printf("nMusics = %s ... Done\n", nMusics);
    char* totalArtist = wrapTotalArtist(query6Data->wrap);
    //printf("totalArtist = %s ... Done\n", totalArtist);
    char* totalDay = wrapTotalDay(query6Data->wrap);
    //printf("totalDay = %s ... Done\n", totalDay);
    char* totalAlbum = wrapTotalAlbum(query6Data -> wrap);
    //printf("totalAlbum = %s ... Done\n", totalAlbum);
    char* totalHour = wrapTotalHour(query6Data->wrap);
    ...
    if(n != NULL){
        ...
        while (aux->artistsTimes != NULL && nInt>0){
            // Lista a lista
            char* totalArtist2 = wrapTotalArtist(aux);
            char* totalMusics2 = wrapTotalMusics(aux);
